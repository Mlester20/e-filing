...
require_once __DIR__ . '/BaseModel.php';
    ...
    class UpdateProfileModel extends BaseModel {
        ...
        public function getUserById($id) {
            try {
                $sql = "SELECT id, name, email, role, profile_picture, created_at FROM {$this->users} WHERE id = ? LIMIT 1";
        ...
        public function updateProfile($id, $name, $email, $password = null) {
            try {
                if ($password === null) {
                    ...
                    $sql = "UPDATE {$this->users} SET name = ?, email = ?, updated_at = NOW() WHERE id = ?";
                    $stmt = $this->con->prepare($sql);
                    $stmt->bind_param("ssi", $name, $email, $id);
                } else {
                    // Update name, email, and password
                    $hashedPassword = password_hash($password, PASSWORD_BCRYPT);
                    $sql = "UPDATE {$this->users} SET name = ?, email = ?, password = ?, updated_at = NOW() WHERE id = ?";
                    $stmt = $this->con->prepare($sql);
